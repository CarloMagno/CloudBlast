...
public abstract class BLAST {
    ...
    public Alignment[] align(Sequence querySeq, Sequence subjectSeq) {
        ...
        for (int i = 0; i < hits.size(); i++) {
            HSP temp = hits.get(i);
            int currScore, fScore = 0, rScore = 0, alignscore = 0, 
                difference = 0, startRange = 0, endRange = 0;

            //extend forward
            queryIndex = temp.getQPos() + wordLength;
            subjectIndex = temp.getSPos() + wordLength;
            ...
            difference = 0;
            alignscore = 0;
            queryIndex = temp.getQPos() - 1;
            subjectIndex = temp.getSPos() - 1;
            ...
            currScore = rawScore(getRange(query, temp.getQPos() - startRange, temp.getQPos() + wordLength + endRange),
                                 getRange(subject, temp.getSPos() - startRange, temp.getSPos() + wordLength + endRange));
            ...
            if ((currScore > 0) && (eScore <= eCutoff)) {
                ...
                alignment.setMyQueryStart(temp.getQPos() - startRange);
                alignment.setMyQueryFinish(temp.getQPos() + wordLength + endRange);
                alignment.setMySubjectStart(temp.getSPos() - startRange);
                alignment.setMySubjectFinish(temp.getSPos() + wordLength + endRange);
